...
class SociosModel(object):
    ...
    def get_partner(self, id):
        try:
            self.cur.execute("SELECT * FROM empresas_socios es LEFT JOIN  empresas e ON e.id = es.id_empresa WHERE  es.id = '{}' AND  e.status = 'Ativo';".format(id))
    ...
    def get_partners(self, id):
        try:
            self.cur.execute("SELECT  es.id, es.nome, es.cpf, es.participacaoCapitalSocial FROM empresas_socios es LEFT JOIN  empresas e ON  e.id = '{}' AND  e.id = es.id_empresa  WHERE e.status = 'Ativo';".format(id))
